...
export default function WeekAhead() {
    ...
    const currentTime: number = parseInt(weatherData.list[0].dt_txt.slice(11, -3));
    ...
    for (let i = 0; i < len; i++) {
        ...
        for (let j = startIndex; j < endDayIndex && j < weatherData.list.length; j++) {
            const description = `${weatherData.list[j].weather[0].description.slice(0, 1).toUpperCase()}${weatherData.list[0].weather[0].description.slice(1)}`;
            const date = `${weatherData.list[j].dt_txt.slice(0, 10)}`
            weatherData.list.slice(startIndex, endDayIndex).map(item => {
                weatherItems.push({ icon: item.weather[0].icon, description: description, date: date})
            });
        }
    ...
    <>
        {weekAheadIcons.map(item => {
            ...
            return (
                <Icon key={id} id={id} icon={icon} description={weatherInfo} />
            )
